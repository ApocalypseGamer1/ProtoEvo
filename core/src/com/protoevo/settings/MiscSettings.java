...
public class MiscSettings extends Settings {
    ...
    // Which physics engine drives the sim.
    //   "box2d" — original libGDX Box2D wrapper. Mature C-port, single-threaded
    //             but no allocation in hot loops. Default and recommended.
    //   "jolt"  — custom pure-Java parallel circle-physics engine. Theoretically
    //             scales with cores; empirically SLOWER than Box2D at typical
    //             sim sizes (~500-2000 cells) due to parallelStream overhead
    //             and per-step allocation. Lives in
    //             `core/src/com/protoevo/physics/jolt/` for future optimization
    //             work but not recommended for normal use.
